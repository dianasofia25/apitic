...
public class ManualRadios extends AppCompatActivity {

    TextView t1;
    ImageView image;
    PDFView pdfViewRadio;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_manualradios);
        getSupportActionBar().setDisplayHomeAsUpEnabled(true);

        StorageReference ManualR1C = storageReference.child("Manuales/compressed/"+"ManualR1.pdf");

        pdfViewRadio = (PDFView) findViewById(R.id.pdfviewRadio);
        t1 = (TextView) findViewById(R.id.textR1);
        String value = getIntent().getExtras().getString("R1"); // recibir bandera del fragment Gallery

        if (value.equals("1")){
            if(isInternetAvailable()){
                ManualR1C.getDownloadUrl().addOnSuccessListener(new OnSuccessListener<Uri>() {
                    @Override
                    public void onSuccess(Uri uri) {
                        String URL = uri.toString(); // obtener la URL de la firebase
                        t1.setText(URL);
                        new VistaPDF().execute(URL); // cargar la URL usando la clase VistaPD
                        Toast.makeText(ManualRadios.this,"Actualizado",Toast.LENGTH_SHORT).show();
                    }
                }).addOnFailureListener(new OnFailureListener() {
                    @Override
                    public void onFailure(@NonNull Exception e) {
                        Toast.makeText(ManualRadios.this,"Hubo un error en la actualización "
    ...
    class VistaPDF extends AsyncTask<String,Void, InputStream> { // tarea asincrona para mostrar el pdf
        ...
        protected void onPostExecute(InputStream inputStream) {
            pdfViewRadio.fromStream(inputStream).load(); // se muestra el pdf en la vista pdf
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if (id == R.id.action_download) {
            ...
            StorageReference ManualR1 = storageReference.child("Manuales/"+"ManualR1.pdf");

            ManualR1.getDownloadUrl().addOnSuccessListener(new OnSuccessListener<Uri>() {
                @Override
                public void onSuccess(Uri uri) {
                    String URL = uri.toString(); // obtener la URL de la firebase
                    downloadFile(ManualRadios.this,"ManualR1",".pdf",DIRECTORY_DOWNLOADS,URL);
                    Toast.makeText(ManualRadios.this,"Descargando...",Toast.LENGTH_SHORT).show();
                }
            }).addOnFailureListener(new OnFailureListener() {
                @Override
                public void onFailure(@NonNull Exception e) {
                    Toast.makeText(ManualRadios.this,"Error! "
                            +e.getMessage(),Toast.LENGTH_SHORT).show();
                }
            });
            return true;
    ...
    private void readPdf(){
        image = (ImageView) findViewById(R.id.imageR1);
        File path = new File(Environment.getExternalStorageDirectory().getPath()
                + "/Android/data/com.example.apitic/files/Download/ManualR1.pdf");
        if(path.exists()){
            PDFView pdfView = findViewById(R.id.pdfviewRadio);
            Toast.makeText(ManualRadios.this,path.toString(),Toast.LENGTH_SHORT).show();
